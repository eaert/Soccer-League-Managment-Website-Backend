...
router.get("/teamFullDetails/:teamName", async (req, res, next) => {
  ...
  try {
    team_details.team = (await teams_utils.teamDetails(req.params.teamName))[0];
    team_details.games = await games_utils.getGamesByTeamName(req.params.teamName);
    team_details.players = await players_utils.getPlayersByTeam(team_details.team.teamID);
    team_details.coach = await coaches_utils.coachDetailsByTeamName(team_details.team.teamID);
